...
export const getProfile = async () => {
    ...
    const user = {
        ...
        basicInfo: basicInfo,
        patientData : {
            bloodGluscose : bloodGluscoseGet(id),
...
export const bloodGluscosePost = async (data) => {
    const id = patientId;
    const display = "Glucose [Moles/volume] in Capillary blood by Glucometer";
    const text = data.mealTime;
    const effectiveDateTime = data.time + "+00:00";
    const issued = data.time + "+00:00";
    //blood glucose code
    const code = "14743-9";
    //patient
    ...
    //CHECK LEVEL
    let interpretationCode, interpretationDisplay;
    ...
    console.log("here is post data", sendDate)
    try {
        const response = await axios.post('http://localhost:8080/fhir/Observation', sendDate);
